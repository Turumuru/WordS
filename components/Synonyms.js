...
function Synonyms() {
    ...
    const options = {
        method: 'GET',
        url: `https://wordsapiv1.p.rapidapi.com/words/${searchTerm}/synonyms`,
        headers: {
    ...
    useEffect(() => {
        axios.request(options).then((resp) => {
            console.log(resp.data.synonyms);
            const word = resp.data.synonyms;
            console.log(word);
            Setwords({
                words: word
            });
