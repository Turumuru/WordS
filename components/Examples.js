...
function Examples() {
    ...
    const options = {
        method: 'GET',
        url: `https://wordsapiv1.p.rapidapi.com/words/${searchTerm}/examples`,
        headers: {
    ...
    useEffect(() => {
        axios.request(options).then((resp) => {
            console.log(resp.data.examples);
            const word = resp.data.examples;
            console.log(word);
            Setwords({
                words: word
            });
    ...
    if (wor) {
        ...
        return (
            <LayoutComponent>
                ...
                <div style={{paddingLeft: "300px", paddingTop: "100px"}}>
                    <h1>Примеры в предложении для Слова {searTerm}</h1>
                    <input
        ...
        return (
            <LayoutComponent>
                ...
                <div style={{paddingLeft: "300px", paddingTop: "100px"}}>
                    <h1>Примеры в предложении для Слова</h1>
                    <input
                        style={{backgroundColor: '#4B4747', width: '650px', height: '25px'}}
                        type="text"
                        placeholder="Search..."
